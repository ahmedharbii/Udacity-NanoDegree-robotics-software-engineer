...
{
	ROS_INFO("DriveToTargetRequest received - X:%1.2f, Wz:%1.2f", (float)req.linear_x, (float)req.angular_z);

	// Create a motor_command object of type geometry_msgs::Twist
        geometry_msgs::Twist motor_command;

        // Set wheel velocities as requested
	...
	res.msg_feedback = "linear_x set - x: " + std::to_string(req.linear_x) + " & angular_z, wz: " + std::to_string(req.angular_z);
